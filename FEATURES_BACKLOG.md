...
## Recipe-driven cart fill

**Status:** Deferred — content-heavy, but distinctly African
behaviour.

**Why it matters:** African cooking is recipe-driven and
intergenerational. "How does Mum make jollof?" is a daily
question. A recipe section that tags every ingredient to an
Afrizonemart product — and lets the customer tap "Add all
ingredients to cart" — turns content into commerce. Increases
basket size, drives discovery of products customers wouldn't
have searched for, gives the site editorial substance.

**What it needs (rough scope):**
- New `Recipe` Prisma model — title, slug, coverImageUrl, body
  (markdown), regionalOriginCountry, servings, cookTime,
  difficulty (easy/medium/hard), createdAt.
- `RecipeIngredient` — recipeId, productId (FK to Product),
  quantity (free text — "2 cups", "500g"), notes.
- Admin: `/admin/recipes` for curation. Interns can submit
  recipes (similar to existing intern queue), admin approves
  before publish.
- Storefront: `/recipes/[slug]` page renders the recipe + an
  "Add all to cart" CTA that pushes every linked product into
  the cart store.
- Discovery: recipe shelf on category pages ("Try a Yoruba
  jollof — every ingredient in the recipe is in your cart with
  one tap"); recipe of the week in the Daily Stories tray (see
  earlier entry).
- Recipe stub on product pages: "This product appears in 5
  recipes. View them."
- Community submissions later: customers submit their own
  family recipes, get a small loyalty bonus when one is
  approved. Becomes a brand-building content engine.

**Trigger to promote:** When we have at least one editorial
content writer / intern who can ship 5–10 recipes per week
for the first 2 months (need a critical mass of recipes before
discovery surfaces feel populated).

Added: 2026-05-12

---

## "Auntie's Picks" — community curator program

**Status:** Deferred — needs a critical mass of customers first.

**Why it matters:** African shopping is high-trust, low-search.
People buy what their auntie / mum / cousin recommends, not what
an algorithm recommends. Letting trusted community members
publish curated collections ("Auntie Folake's Lagos kitchen
essentials", "Pastor's wife Sunday dinner picks") turns
informal word-of-mouth into a discoverable, persistent surface
on the site.

**What it needs (rough scope):**
- New `Curator` Prisma model — userId, displayName, bio,
  avatarUrl, isVerified, joinedAt.
- `CuratedCollection` Prisma model — curatorId, title, slug,
  coverImageUrl, description, isPublished.
- `CollectionItem` — collectionId, productId, displayOrder,
  curatorNote (short text).
- Curator application flow: existing customers can apply at
  `/account/become-a-curator`. Admin reviews + approves.
  Verified curators get a checkmark.
- Curator profile pages: `/curators/[displayName]` — bio,
  avatar, all their collections.
- Collection pages: `/collections/[slug]` — looks like a shop
  page but with the curator's intro + per-product notes.
- Discovery: featured curator on homepage, "Curators you might
  like" section, daily-stories integration.
- Optional revenue share: 2–5% of orders from a curator's
  collection links go to the curator (paid out via Continental
  Rewards wallet).

**Trigger to promote:** Promote when (a) we have at least 5
genuine community members who'd seriously curate (Magnus
network or seeded through existing happy customers), and (b)
Continental Rewards wallet exists to pay revenue-share. Pre-
launch this can also be a marketing campaign — invite 5
trusted aunties / chefs / pastors to curate.

Added: 2026-05-12

---

## Country-of-the-week / Country spotlight

**Status:** Deferred — editorial, lightweight, ships fast.

**Why it matters:** Pan-African is the differentiation — but
the homepage doesn't currently celebrate that. A weekly
"Country in focus" spotlight tells a 30-second story about
one African country, surfaces 8–12 hand-picked products from
that country, and rotates each Monday. Free editorial content,
free pan-African brand reinforcement, free reason to come back
weekly.

**What it needs (rough scope):**
- Reuse the existing CMS page builder — each country gets a
  block (hero image, intro paragraph, "About this country"
  history blurb, embedded products via product picker, optional
  Meet-the-maker stories from that country).
- New `CountrySpotlight` Prisma model — countryCode (FK to ISO
  list), pageBuilderSlug, startsAt, endsAt, isActive.
- Cron on Monday 6am: deactivates last week's, activates this
  week's. Or just a schedule-aware boot check.
- Homepage hero band: "🇰🇪 This week: Kenya — discover 12
  Kenyan kitchen essentials [Explore]". Below: a thumbnail
  carousel of the 8–12 products.
- Email newsletter integration — Monday morning newsletter
  pushes the new country.
- Archive page: `/countries` shows past spotlights, becomes
  evergreen SEO content over time.

**Trigger to promote:** Promote anytime. Editorial overhead is
low (one staff member can write/curate one country per week),
ROI is repeatable evergreen content. Could ship in 1–2 weeks.

Added: 2026-05-12

---

## Re-stock predictions ("you usually run out of...")

**Status:** Deferred — data-light, retention-heavy.

**Why it matters:** Staple-grocery buyers buy the same things
on roughly the same cadence. If we observe that a customer
bought garri 28 days ago, 26 days before that, and 31 days
before that — we can confidently nudge them at day 23 with
"You usually run out of garri every 28 days. Order before you
hit empty?" Tiny push, huge retention lift. Compound effect
with subscriptions: customers who don't want full subscriptions
still benefit from the reminder.

**What it needs (rough scope):**
- Scheduled job per customer (daily or every-other-day cron):
  for each frequently-purchased product, compute the median
  days between purchases. If `now - lastPurchaseDate > median
  - 5 days`, queue a notification.
- Confidence threshold: only nudge if the customer has bought
  the product 3+ times so we have a real cadence signal.
- Notification format: SMS or push — "Your usual garri (Tom
  Brown 500g) — you usually run out around now. Re-order in
  one tap? [Yes / Maybe Later]".
- "Maybe later" snoozes for 5 days. "Yes" pre-fills cart with
  the same SKU + quantity.
- Customer preference: an opt-out per product OR a global
  "stop these predictions" toggle in `/account/notifications`.
- Account dashboard: "Likely to run out soon" widget showing
  3–5 candidates with one-tap reorder.

**Trigger to promote:** Once a meaningful number of customers
have 3+ purchases of the same SKU (need real cadence data).
Probably ~6 months post-launch when the customer base + repeat
rate are big enough. Pairs strongly with the "One-tap re-order
...
## Charity round-up / Round to support Africa

**Status:** Deferred — small scope, big brand moment.

**Why it matters:** Customers feel good. Brand stands for
something. Free PR. Charity gets meaningful funding from small
per-order amounts. At scale (₦100 average per order × 10k
orders/month) that's ₦1M/month flowing to a curated African
charity — quotable in press, retention-positive for socially-
conscious customers, especially diaspora.

**What it needs (rough scope):**
- New `Charity` Prisma model — name, description, logoUrl,
  cause (education / health / women / environment / etc.),
  countryFocus, isActive.
- Admin curates 3–5 charities. Customer picks one as their
  "current charity" in account settings.
- Checkout addition: a toggleable line item "Round up ₦X to
  nearest ₦100 and donate to [charity]". Default OFF on first
  order, ON once a customer opts in.
- Optional: bigger fixed-amount additions ("Add ₦500 to my
  donation").
- Quarterly: admin tallies, releases funds to charities,
  publishes a transparency report. Page at `/impact` shows
  total raised, per-charity breakdown, impact stories.
- Donation receipts emailed to customer for any qualifying
  tax-deductible amount (where the charity is registered).

**Trigger to promote:** Once we have a partnership lined up
with 2–3 reputable African-cause charities. Charity selection
matters more than the build — the wrong charity makes this
feel performative; the right one makes it iconic.

Added: 2026-05-12

---

## Friends' wishlists — social gifting

**Status:** Deferred — depends on wishlist already shipping
(it has — `/account/wishlist`).

**Why it matters:** Gift-giving is a huge African cultural moment
— weddings, naming ceremonies, graduations, festivals. Letting
customers share their wishlist as a link ("Hey, my birthday is
next week — pick something") solves the "what to give" problem
for the gifter and removes the social awkwardness of asking for
gifts directly. Cousin shares wishlist on WhatsApp family group,
family member buys from it — Afrizonemart picks up the order.

**What it needs (rough scope):**
- Add `isShared`, `shareToken` (random URL-safe), `shareTitle`
  (optional, "My birthday wishes") fields to the existing
  WishlistItem / Wishlist model.
- New public page `/wishlist/[shareToken]` — read-only view of
  someone else's wishlist. Shows items with prices and a "Buy
  this gift for [name]" CTA.
- Buying flow: gift-buyer adds the item to their own cart, sees
  a "This is a gift for [recipient name]" tag throughout
  checkout. Optional gift note. Ships to the recipient's
  default address if they have one on file with us (privacy-
  preserved via the Recipient model from "Send to my mum") OR
  to a manually-entered address.
- Wishlist-owner notification: when someone marks an item as
  "Gifted" (or buys it), the owner sees a heart on their list
  but not which friend bought it (surprise-preserving). They
  can choose to see who bought what after delivery.
- Privacy: shareToken is hard to guess (256-bit random); the
  owner can revoke + regenerate at any time.

**Trigger to promote:** Promote whenever; small build, pairs
with "Send to my mum" infrastructure. Could be a Mother's Day
campaign launch.
